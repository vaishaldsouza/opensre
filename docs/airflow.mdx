...
Related (Tracer, not Airflow API): `get_airflow_metrics` pulls orchestration metrics from Tracer when a `trace_id` is available. See [integration configuration](/integrations-overview) for connection guidance.
