...
Separately from typed-command history, OpenSRE records each agent turn — the
submitted prompt and final response — in the interactive shell,
`opensre ask`, gateway chat, and scheduled or embedded agent runs. Failures and
...
1. **Local debugging / `/resume`**: appended as JSON Lines to
   ...
   can restore conversation context. Gateway logs use the organization and user
   conversation directory rather than the host-wide default. An explicit
   `OPENSRE_PROMPT_LOG_PATH` overrides that location.
2. **Product analytics**: forwarded through the first-party OpenSRE app endpoint
   as an `$ai_generation` event
...
| Remote forwarding | **on** | Turns are also sent as a product-analytics `$ai_generation` event. |
...
| Variable | Default | Effect |
| --- | --- | --- |
| `OPENSRE_PROMPT_LOG_DISABLED` | `0` | Set to `1` to disable prompt/response logging entirely (both local file and remote analytics). |
| `OPENSRE_PROMPT_LOG_LOCAL_DISABLED` | `0` | Set to `1` to skip the local JSONL file while leaving remote forwarding (if enabled) unaffected. |
...
The prompt/response log has the same caveats, plus one more: with remote
